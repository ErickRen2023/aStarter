...
// ========== 7 天轮换状态 ==========
const BING_DAY_STATE_KEY = 'bingDayState';   // { index, date } — date 为做出选择时的日期
let bingDayIndex = 0;   // 0=今天, 1=昨天, ..., 6=6天前
let bingDayCount = 7;   // 实际可用天数

/**
 * 从 chrome.storage.sync 恢复上次切换的天数偏移；
 * 仅在做出选择的当天内有效，跨天则重置为当天
 * （issue #11 https://github.com/ErickRen2023/aStarter/issues/11）
 */
async function restoreBingDayIndex() {
  try {
    const stored = await chrome.storage.sync.get(BING_DAY_STATE_KEY);
    const state = stored?.[BING_DAY_STATE_KEY];
    const today = new Date().toISOString().split('T')[0];
    if (state && Number.isInteger(state.index) && state.date === today) {
      bingDayIndex = Math.max(0, Math.min(6, state.index));
    } else {
      bingDayIndex = 0;
    }
  } catch {
...
/**
 * 持久化当前天数偏移及选择日期（fire-and-forget）
 */
function persistBingDayIndex() {
  const today = new Date().toISOString().split('T')[0];
  chrome.storage.sync.set({
    [BING_DAY_STATE_KEY]: { index: bingDayIndex, date: today }
  }).catch(() => {});
}
 ...
 * Bing 默认返回 1920x1080，替换为 UHD 可获得 4K 分辨率
 */
function toUHDUrl(url) {
  return url.replace(/_1920x1080/g, '_UHD');
}
...
export async function downloadAndCacheBing() {
  ...
  try {
    // 优先 UHD，失败则回退普通分辨率
    let resp = await fetch(info.uhdUrl);
    if (!resp.ok) resp = await fetch(info.url);
...
export async function downloadAndCacheBingWeekly() {
  ...
  for (const img of images) {
    try {
      // 优先 UHD，失败则回退普通分辨率
      let resp = await fetch(img.uhdUrl);
      if (!resp.ok) resp = await fetch(img.url);
      if (!resp.ok) continue;
      const blob = await resp.blob();
...
export async function render(cfg) {
  settings = { ...cfg };
  await restoreBingDayIndex();   // 恢复上次切换的天数偏移（issue #11 https://github.com/ErickRen2023/aStarter/issues/11）
  stopCarousel();
...
async function renderBing() {
  ...
  // 没有缓存，实时 fetch 今天（优先 UHD）
  const info = await fetchBingDaily();
  if (info) {
    // 尝试加载 UHD，失败则回退普通分辨率
    const testImg = new Image();
    testImg.onload = () => { setImage(info.uhdUrl); };
    testImg.onerror = () => { setImage(info.url); };
    testImg.src = info.uhdUrl;
    // 后台下载完整的 7 天缓存
    downloadAndCacheBingWeekly().catch(() => {});
  } else {
    // 降级：纯色背景
